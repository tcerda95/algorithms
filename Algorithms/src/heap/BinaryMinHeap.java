...
public class BinaryMinHeap <T> implements PriorityQueue<T> {
    ...
    @Override public void enqueue (T elem, int priority) {
    ...
    @Override public int getPriority (T elem) {
    ...
    @Override public void decreasePriority (T elem, int priority) {
        Integer index = indexMap.get(elem);
        if (index == null)
            throw new NoSuchElementException("No existe " + elem);
    ...
    public int minPriority () {
        if (isEmpty()) {
            throw new NoSuchElementException("Cola vacÃ­a");
        }
